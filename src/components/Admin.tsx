...
import {
  ...
  RefreshCw, Search, Send, Settings, ShieldCheck, Sparkles, Target, Trash2, TrendingUp, UserCheck,
  UserMinus, Users
} from "lucide-react";
...
function AdminOverview() {
  ...
  return <>
    <PageHeader eyebrow="Operations workspace" title="Good morning, Maya." description="Content quality is stable. Two learner reports need editorial review today." actions={<><button className="btn btn-secondary" onClick={()=>downloadFile("stepwise-admin-report.json",JSON.stringify(state,null,2))}><Download/> Export report</button><Link className="btn btn-brand" href="/admin/questions"><FilePlus2/> New question</Link></>}/>
    <section className="stat-grid four admin-stat-grid">
      <StatCard label="Active learners" value={activeUsers} detail=" of 6 demo accounts" trend="+8.4% " icon={<Users/>}><Sparkline values={[28,32,31,38,42,46,51]} height={34}/></StatCard>
      ...
      <StatCard label="Open reports" value={openReports} detail=" median age 19 hours" icon={<Flag/>}><span className="admin-stat-link">Review queue <ArrowRight/></span></StatCard>
      <StatCard label="Monthly recurring revenue" value="$42.8k" detail=" demo forecast" trend="+12.2% " icon={<CircleDollarSign/>}><Sparkline values={[21,25,27,31,34,38,43]} height={34}/></StatCard>
    </section>

    <section className="admin-overview-grid">
      <article className="panel admin-growth-card"><header><div><div className="card-kicker"><TrendingUp/> Platform activity</div><h2>Learner engagement</h2><p>Daily active learners and completed question blocks.</p></div><select defaultValue="30"><option value="30">Last 30 days</option><option value="90">Last 90 days</option></select></header><div className="admin-growth-chart">{[42,48,45,56,61,59,72,68,77,84,81,92].map((value,index)=><span key={index}><i style={{height:`${value*1.6}px`}}/><b style={{height:`${Math.max(16,value*1.1)}px`}}/></span>)}</div><footer><span><i/> Active learners</span><span><i/> Completed blocks</span><b>3,942 questions answered this week</b></footer></article>
      <article className="panel admin-health-card"><header><div className="card-kicker"><ShieldCheck/> Content health</div><Badge tone="success" dot>Healthy</Badge></header><div className="admin-health-score"><Donut value={94} size={148} detail="health"/><div><h2>94 / 100</h2><p>Based on report volume, review freshness, blueprint coverage, and learner discrimination signals.</p></div></div><div className="admin-health-bars">{[["Medical review freshness",96],["Blueprint coverage",91],["Report resolution",88],["Item performance",94]].map(([label,value])=><div key={String(label)}><Progress value={Number(value)} label={String(label)} showValue/></div>)}</div></article>
    </section>
...
function BillingAdmin() {
  ...
  const [toast,setToast]=useState("");
  const revenue=[26,29,31,33,35,38,43];
  return <>
    ...
    <section className="stat-grid four"><StatCard label="MRR" value="$42,840" trend="+12.2% " detail="month over month" icon={<CircleDollarSign/>}/><StatCard label="Paid accounts" value={state.adminUsers.filter(u=>u.plan!=="Trial").length} detail="83% conversion" icon={<UserCheck/>}/><StatCard label="Trial conversion" value="38%" trend="+4.1% " detail="last 30 days" icon={<TrendingUp/>}/><StatCard label="Churn" value="2.7%" trend="-0.6% " detail="month over month" icon={<UserMinus/>}/></section>
    <section className="billing-main-grid"><article className="panel revenue-card"><header><div><div className="card-kicker"><BarChart3/> Revenue trend</div><h2>Recurring revenue</h2></div><select value={period} onChange={e=>setPeriod(e.target.value)}><option>Monthly</option><option>Quarterly</option><option>Annual</option></select></header><div className="revenue-chart">{revenue.map((value,index)=><div key={index}><span>${value}k</span><i style={{height:`${value*5}px`}}/><small>{["Jan","Feb","Mar","Apr","May","Jun","Jul"][index]}</small></div>)}</div><footer><b>$42.8k current MRR</b><span>Projected annual run rate: $514k</span></footer></article><article className="panel plan-mix"><header><div className="card-kicker"><Layers3/> Plan mix</div></header><div className="plan-mix-donut"><Donut value={62} size={150} detail="Pro + Inst."/><div>{[["Pro",38,"brand"],["Institution",24,"info"],["Core",27,"success"],["Trial",11,"warning"]].map(([label,value,tone])=><p key={String(label)}><Badge tone={tone as "brand"|"info"|"success"|"warning"} dot>{label}</Badge><b>{value}%</b></p>)}</div></div><button className="btn btn-secondary btn-block" onClick={()=>{setToast("Plan configuration opened in demo mode");setTimeout(()=>setToast(""),1600)}}>Manage plans</button></article></section>
