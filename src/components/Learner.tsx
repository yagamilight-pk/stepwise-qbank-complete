...
import { estimateLearnerAbility } from "@/lib/psychometrics";
...
function AnalyticsPage() {
  ...
  const ability=estimateLearnerAbility(questions,attempts);
  ...
  return <>
    <PageHeader title="Analytics" description="See measured performance, uncertainty, calibration, pacing, and adaptive-learning signals from your saved attempts." actions={<div className="header-selects"><select aria-label="Analytics exam" value={step} onChange={e=>setStep(e.target.value as Step)}><option>Step 1</option><option>Step 2 CK</option></select><select aria-label="Analytics date range" value={range} onChange={e=>setRange(e.target.value)}><option>7 days</option><option>30 days</option><option>All time</option></select></div>}/>
    ...
    {tab==="Overview"&&<>
      <section className="stat-grid four analytics-stats">
        <StatCard label="Overall accuracy" value={`${accuracy}%`} detail={`${attempts.length} measured response${attempts.length===1?"":"s"}`} icon={<Target/>}/>
        ...
        <StatCard label="Average pace" value={avgTime?`${Math.floor(avgTime/60)}:${String(avgTime%60).padStart(2,"0")}`:"—"} detail="from submitted responses" icon={<Clock3/>}/>
        <StatCard label="Rasch ability" value={ability.responses>=5?`${ability.theta>=0?"+":""}${ability.theta.toFixed(2)} θ`:"Calibrating"} detail={ability.responses>=5?`SE ${ability.standardError.toFixed(2)} · ${ability.responses} latest items`:`${Math.max(0,5-ability.responses)} more unique items needed`} icon={<Gauge/>}/>
      </section>
    ...
    {tab==="Peers"&&<section className="peer-page-grid">
      <article className="panel peer-percentile-card"><div className="card-kicker"><Trophy/> Privacy-safe cohort benchmark</div><div className="peer-percentile-main"><Donut value={Math.min(100,peer.sampleSize/peer.minimumSample*100)} size={190} detail="cohort readiness"/><div><Badge tone="neutral">{peer.band}</Badge><h2>No invented percentile.</h2><p>Stepwise will publish a percentile only after at least {peer.minimumSample} eligible learners contribute a comparable exam, mode, and time window. Until then, the measured personal metrics below remain available.</p></div></div><div className="peer-comparison-grid"><div><span>Your accuracy</span><b>{peer.accuracy}%</b><small>{attempts.length} saved responses</small></div><div><span>Your pace</span><b>{peer.averageTime?`${peer.averageTime}s`:"—"}</b><small>Median-free personal measure</small></div><div><span>Eligible cohort</span><b>{peer.sampleSize}</b><small>{peer.minimumSample} required</small></div><div><span>Published percentile</span><b>—</b><small>Suppressed below threshold</small></div></div></article>
      <article className="panel peer-distribution-card"><header><div><h2>Cohort distribution withheld</h2><p>A real distribution requires sufficient independent learners, comparable sessions, consent, and anti-gaming checks.</p></div><Badge tone="info">Quality gate</Badge></header><div className="locked-signal-preview"><i/><i/><i/><span>Stepwise does not manufacture a comparison curve when the sample is insufficient.</span></div></article>
    </section>}
  </>;
}
