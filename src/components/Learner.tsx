...
import {
  ...
  Calendar, CalendarDays, Check, ChevronRight, CircleAlert, Clock3, Flame,
  Gauge, Layers3, ListFilter, LockKeyhole, MoreHorizontal, Play, RefreshCw, Search,
...
function DashboardPage() {
  ...
  return <>
    ...
    <section className="stat-grid four">
      <StatCard label="Questions today" value={answeredToday} detail={` of ${state.settings.dailyGoal} daily goal`} icon={<BookOpen/>}><Progress value={(answeredToday/state.settings.dailyGoal)*100}/></StatCard>
    ...
    <section className="dashboard-main-grid">
      <article className="panel todays-plan"><header><div><div className="card-kicker"><CalendarDays/> Today&apos;s plan</div><h2>{completed} of {todayTasks.length} tasks complete</h2></div><Link href="/app/study-plan">Full plan <ArrowRight/></Link></header><Progress value={todayTasks.length ? (completed/todayTasks.length)*100 : 0}/><div className="task-list">{todayTasks.map(task=><div key={task.id} className={task.completed?"task-row completed":"task-row"}><button onClick={()=>{dispatch({type:"TOGGLE_TASK",id:task.id});showToast(task.completed?"Task reopened":"Task completed")}}>{task.completed?<Check/>:<span/>}</button><span className={`task-type task-type-${task.type.toLowerCase()}`}>{task.type === "Questions"?<BookOpen/>:task.type === "Flashcards"?<Layers3/>:<BookCheck/>}</span><div><b>{task.title}</b><p>{task.detail}</p></div><span>{task.minutes} min</span>{!task.completed&&<Link href={task.type==="Flashcards"?"/app/flashcards":"/app/qbank"}><ArrowRight/></Link>}</div>)}</div></article>
...
function QBankPage() {
  ...
  return <>
    <PageHeader title="Question bank" description="Build exam-faithful blocks or let the adaptive engine choose your highest-value questions." actions={<><button className="btn btn-secondary" onClick={()=>setBuilderHelp(true)}><CircleAlert/> How selection works</button><button className="btn btn-brand" onClick={()=>start()} disabled={!selectedCount}><Play/> Start block</button></>}/>
    <div className="page-tabs"><button className={tab==="build"?"active":""} onClick={()=>setTab("build")}>Build a block</button><button className={tab==="library"?"active":""} onClick={()=>setTab("library")}>Browse library</button><button className={tab==="history"?"active":""} onClick={()=>setTab("history")}>Session history</button></div>
...
function AnalyticsPage() {
  ...
  return <>
    <PageHeader title="Analytics" description="See what is improving, what is fragile, and how your performance compares with the seeded learner cohort." actions={<div className="header-selects"><select value={step} onChange={e=>setStep(e.target.value as Step)}><option>Step 1</option><option>Step 2 CK</option></select><select value={range} onChange={e=>setRange(e.target.value)}><option>7 days</option><option>30 days</option><option>All time</option></select></div>}/>
    <div className="page-tabs analytics-tabs">{["Overview","Systems","Confidence","Pacing","Peers"].map(item=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}>{item}</button>)}</div>
    ...
    {tab==="Overview"&&<>
      ...
      <section className="analytics-grid-main">
        ...
        <article className="panel readiness-breakdown">
          ...
          <div className="readiness-donut-wrap"><Donut value={readiness.score} size={160} detail="readiness"/><div><Badge tone={readiness.score>=70?"success":"warning"}>{readiness.score>=70?"On track":"Needs attention"}</Badge><h3>{readiness.score>=70?"Goal alignment is strong":"Coverage can move the score"}</h3><p>Readiness blends accuracy, unique coverage, calibration, and system mastery.</p></div></div>
      ...
      <section className="analytics-bottom-grid">
        ...
        <article className="panel confidence-insight">
          <div className="confidence-top"><span><BrainCircuit/></span><Badge tone="warning">Calibration</Badge></div>
