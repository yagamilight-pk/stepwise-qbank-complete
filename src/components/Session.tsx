...
import {
  ArrowLeft, ArrowRight, Bookmark, BrainCircuit, Calculator, Check, CheckCircle2, CircleHelp,
...
export function SessionPage() {
  ...
  return <div className="session-page">
    <header className="session-toolbar"><div><button className="session-exit" onClick={()=>setExitOpen(true)}><ArrowLeft/> Exit</button><div className="session-progress-title"><b>{config.step} · {config.mode}</b><span>Question {index+1} of {questions.length}</span></div></div><div className="session-tools"><button onClick={()=>setPaletteOpen(!paletteOpen)}><List/><span>Navigator</span></button><button onClick={()=>setLabOpen(true)}><FlaskConical/><span>Lab values</span></button><button onClick={()=>setCalculatorOpen(true)}><Calculator/><span>Calculator</span></button><button onClick={()=>setPaused(!paused)}>{paused?<Play/>:<Pause/>}<span>{paused?"Resume":"Pause"}</span></button>{state.settings.showTimer&&<div className={remaining<180&&config.mode!=="Tutor"?"session-timer warning":"session-timer"}><Clock3/><b>{config.mode==="Tutor"?formatSeconds(seconds):formatSeconds(remaining)}</b><small>{config.mode==="Tutor"?"elapsed":"remaining"}</small></div>}</div></header>
    <div className="session-progress-bar"><i style={{width:`${((index+(answered?1:0))/questions.length)*100}%`}}/></div>
    {paletteOpen&&<div className="question-palette"><header><b>Question navigator</b><button onClick={()=>setPaletteOpen(false)}><X/></button></header><div>{questions.map((item,itemIndex)=><button key={item.id} className={`${itemIndex===index?"current":""} ${submitted.includes(item.id)?results.find(r=>r.questionId===item.id)?.correct?"correct":"incorrect":""} ${state.flagged.includes(item.id)?"flagged":""}`} onClick={()=>{goTo(itemIndex);setPaletteOpen(false)}}>{itemIndex+1}{state.flagged.includes(item.id)&&<i/>}</button>)}</div><footer><span><i className="answered"/> Answered</span><span><i className="current"/> Current</span><span><i className="flagged"/> Flagged</span></footer></div>}
    {paused&&<div className="paused-overlay"><div><Pause/><h2>Session paused</h2><p>Your timer is stopped and the question is hidden.</p><button className="btn btn-brand" onClick={()=>setPaused(false)}><Play/> Resume session</button></div></div>}
    <main className="question-workspace">
      <aside className="question-meta"><Badge tone="brand">{question.step}</Badge><dl><div><dt>System</dt><dd>{question.system}</dd></div><div><dt>Discipline</dt><dd>{question.discipline}</dd></div><div><dt>Difficulty</dt><dd><Badge tone={question.difficulty==="Hard"?"danger":question.difficulty==="Easy"?"success":"warning"}>{question.difficulty}</Badge></dd></div></dl><div className="session-utility"><button className={state.flagged.includes(question.id)?"active":""} onClick={()=>dispatch({type:"TOGGLE_FLAG",questionId:question.id})}><Flag/> {state.flagged.includes(question.id)?"Flagged":"Flag question"}<kbd>F</kbd></button><button className={state.bookmarks.includes(question.id)?"active":""} onClick={()=>dispatch({type:"TOGGLE_BOOKMARK",questionId:question.id})}><Bookmark/> {state.bookmarks.includes(question.id)?"Bookmarked":"Bookmark"}<kbd>B</kbd></button><button onClick={()=>setNoteOpen(true)}><NotebookPen/> Add note</button><button onClick={()=>setReportOpen(true)}><CircleHelp/> Report issue</button></div><div className="shortcut-card"><b>Keyboard shortcuts</b><span><kbd>1–5</kbd> choose answer</span><span><kbd>F</kbd> flag</span><span><kbd>B</kbd> bookmark</span></div></aside>
      <section className="question-main">
        <div className="question-number"><span>{question.id}</span><div><button title="Highlight stem" className={stemHighlighted?"active":""} onClick={()=>setStemHighlighted(!stemHighlighted)}><Highlighter/></button><button title="Session settings" onClick={()=>showToast("Timer, confidence, and feedback settings are controlled by the block mode")}><Settings2/></button><button title="Question actions" onClick={()=>showToast("Flag, bookmark, note, and report actions are available in the left panel")}><MoreHorizontal/></button></div></div>
        ...
        <div className="choice-list-session">{question.choices.map((choice,choiceIndex)=>{
          ...
          return <div key={choice.id} className={`session-choice ${selectedNow?"selected":""} ${isCorrect?"correct":""} ${isWrong?"wrong":""} ${eliminated?"eliminated":""}`}>
            ...
            <button className="choice-select" disabled={answered} onClick={()=>!answered&&setSelected(current=>({...current,[question.id]:choice.id}))}>
            ...
            <button className="strike-button" disabled={answered} onClick={()=>setStruck(current=>({...current,[question.id]:(current[question.id]||[]).includes(choice.id)?(current[question.id]||[]).filter(id=>id!==choice.id):[...(current[question.id]||[]),choice.id]}))} aria-label="Strike out choice"><span/></button>
          </div>})}</div>
        {!answered&&<div className="confidence-select"><span>How confident are you?</span><div>{([1,2,3,4,5] as Confidence[]).map(level=><button key={level} className={confidence[question.id]===level?"active":""} onClick={()=>setConfidence(current=>({...current,[question.id]:level}))}><b>{level}</b><small>{["Guess","Low","Medium","High","Certain"][level-1]}</small></button>)}</div></div>}
